...
- source_spec: `_bmad-output/implementation-artifacts/spec-2-2-boundaries-domains-and-starter-policies.md`
  summary: Sensitive life-model config free-text (`DomainRenamed.name`, `DomainAdded.name`, `PolicyTemplateAccepted.content`) is encrypted under a per-event erasure scope, so a domain's or a policy template's sensitive history cannot be collectively crypto-shredded as a unit when the erasure / Policy & Boundaries surface (Story 2.7 / later) lands.
  evidence: `packages/db/src/ledger/store.ts` derives `erasureScope = event:<id>` for sensitive events without a `commitmentId`, so each rename/add/accept append gets its own data key. There is no stable `domain:<id>` / `policy:<templateId>` scope grouping. Pre-existing store behavior; harmless under the current MVP (no erasure UI yet) but should be resolved by passing an explicit stable `erasureScope` on these appends before right-to-erasure ships.

- source_spec: `_bmad-output/implementation-artifacts/spec-2-2-boundaries-domains-and-starter-policies.md`
  summary: Rendering the onboarding `boundaries` step reads the full `joint` event stream twice per request — once via `readOnboardingProgress()` (container guard) and again via `readBoundariesStepData()` — each an O(events) decrypt pass, doubling on an append-only stream that grows with every rename/edit forever.
  evidence: `apps/web/src/app/(app)/onboarding/[step]/page.tsx` awaits `readOnboardingProgress()` then `readBoundariesStepData()`, both calling `getStores().ledger.readEvents({ context: 'joint' })`. Correct but redundant; a single read feeding `projectOnboarding` + the three boundaries projections would halve the per-render cost. Same joint-stream-scale family as the spec-2-1 deferred item; low priority at MVP scale.

- source_spec: `_bmad-output/implementation-artifacts/spec-2-3-hard-commitments-and-protection-levels.md`
  summary: The `commitment` projection table no longer carries the full read model — Story 2.3 added required `protectionLevel` + optional `recurrence` to `CommitmentRow` but not to the table (no-migration Block-If), so `readCommitments` now re-derives rows from the whole context-scoped event stream. This decrypts every event of that context (not just commitments) on each read, leaves the still-written `commitment` table vestigial for reads (only its existence-check and a now-unexercised erase-to-table redaction remain), and relies on an undocumented invariant that `reduceCommitment` never reads `current.protectionLevel`/`recurrence` (fabricated as `'hard-commitment'`/`null` placeholders in `applyCommitmentProjection`).
  evidence: `packages/db/src/ledger/store.ts` `readCommitments()` calls `readEventsInternal({ context })` → `projectCommitments`, so an O(all-context-events) decrypt runs per commitments read and over-decrypts unrelated sensitive fields; `applyCommitmentProjection` (`store.ts` ~L333-344) writes placeholder level/recurrence that are provably-safe today but silently corrupt if a future commitment-mutating reducer case reads `current`; the erase-cascade test's stated "projection-table redaction" intent now passes via event-stream crypto-shred instead. All correct and safe at MVP scale (single user, small streams; append still validates), but should be resolved by migrating the `commitment` projection table to carry `protectionLevel`/`recurrence` (restoring a narrow indexed read and a live table-redaction path) when commitment volume or the Policy & Boundaries surface (Story 2.7) warrants it.
